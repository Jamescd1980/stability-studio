#!/usr/bin/env python3
"""Pony V6 i2i v2 — eye cleanup, preserve mouth (canonical sampler + lower denoise)."""
...
# Pony V6: dpmpp_2m + karras, cfg ~5.5 (catalog + MODEL-FAMILIES.md)
SAMPLER = "dpmpp_2m"
SCHEDULER = "karras"
CFG = 5.5
STEPS = 28

# Pony prompt order: subject → action → environment → lighting → style (40–80 words).
# Prefix score_9… added automatically by catalog.
PROMPT = (
...
VARIANTS = [
    {
        ...
        "denoise": 0.30,
        "face_detail": False,
        "note": "preserve mouth — no FaceDetailer; Pony dpmpp_2m/karras",
    },
    {
        "tag": "v2_fd",
        "denoise": 0.26,
        "face_detail": True,
        "note": "lighter denoise + FaceDetailer for eyes only if v2 eyes still weak",
    },
]


def _run_i2i(engine, *, denoise: float, face_detail: bool) -> dict:
    return engine.generate_image_i2i(
...
def main() -> int:
    out_log = MCP_ROOT / "outputs" / "rin_eye_cleanup_pony_v2.json"
    try:
        import requests
        ...
        for v in VARIANTS:
            tag = str(v["tag"])
            print(f"\n=== i2i {tag} denoise={v['denoise']} face_detail={v['face_detail']} ===", flush=True)
            result = _run_i2i(
                engine,
        ...
        payload = {
            ...
            "pony_notes": {
                ...
                "scheduler": SCHEDULER,
                "cfg": CFG,
                "prompt_order": "subject → action → environment → lighting → style",
                "v1_issue": "euler_ancestral i2i default + denoise 0.40 + FaceDetailer likely distorted mouth",
            },
            "runs": runs,
        }
        out_log.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        shutil.copy2(out_log, PROJECT_DIR / "rin_eye_cleanup_pony_v2.json")
