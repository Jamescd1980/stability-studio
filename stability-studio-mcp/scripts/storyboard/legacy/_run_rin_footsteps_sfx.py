...
"""MOSS SFX — Rin footsteps (light dress boots, solid heels, stone path)."""
from __future__ import annotations
...
VARIANTS: list[dict[str, str | int | float]] = [
    {
        "label": "A",
        "prompt": "light heeled boots walking on stone path at a steady calm pace",
        "seed": 441701,
    },
    {
        "label": "B",
        "prompt": "soft dress shoes with solid heels on paved stone walkway, footsteps approaching",
        "seed": 541802,
    },
    {
        "label": "C",
        "prompt": "clear lightweight heel footsteps on stone paving at a slow steady rhythm",
        "seed": 641903,
    },
]
...
def main() -> int:
    out_log = MCP_ROOT / "outputs" / "rin_footsteps_sfx_batch.json"
    try:
        import requests
        ...
        for v in VARIANTS:
            ...
            result = engine.generate_audio(
                ...
                seed=seed,
                filename_prefix=f"audio/rin_footsteps_{label.lower()}",
            )
            saved = Path(result["saved_files"][0])
            dest = delivery_dir / f"Rin_footsteps_option_{label}.mp3"
        ...
        payload = {
            "status": "success",
            "shoes": "lightweight dress boots, solid heels",
            "surface": "stone path",
